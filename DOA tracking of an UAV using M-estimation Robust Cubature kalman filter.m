%% M estimation Robust cubature kalman filter
clear;clc;close all
n=80;
del=1;
R=0.005*eye(4);
...
lamk_b=(inv(pzz)*(zk-zkck))*(zk-zkck)';   %yha transpose hoga shayad end term m

pkb=inv(((lamk_b*pzz)/gama)-pzz+R);
...
ylabel('direction of angle');
xlabel('sampling interval(sec)');
legend('measurement','estimation','predicted','location','best');
suptitle('direction of angle tracking using M-Est Robust cubature kalman filter');
figure;
plot(RMSE);
ylabel('RMSE ');
